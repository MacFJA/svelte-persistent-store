...
export function createEncryptedStorage<T>(
  ...
  try {
    if (
      ...
      (typeof window?.crypto !== "object" || typeof window?.crypto?.subtle !== "object")
    ) {
  ...
  return {
    ...
    removeListener,
    getValue(key: string): T | null {
      Cyrup.encrypt(key, encryptionKey, null, "sps")
        .then((encryptedKey) => {
    ...
    setValue(key: string, value: T): void {
      Cyrup.encrypt(key, encryptionKey, null, "sps").then((encryptedKey) => {
        Cyrup.encrypt(serialize(value), encryptionKey, null, "sps").then((encryptedData) =>
          wrapped.setValue(encryptedKey, encryptedData)
        )
      })
    },
    deleteValue(key: string): void {
