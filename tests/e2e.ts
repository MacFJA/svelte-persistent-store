...
fixture("Svelte Persistent Storage").page("http://localhost:5000")
...
test("Initial state", async (t) => {
  await t
    ...
    .click(clearButton)
    .click(reloadButton)
    .expect(cookieInput.value)
    .eql("John")
    .expect(localInput.value)
    .eql("Foo")
    .expect(sessionInput.value)
    .eql("Bar")
    .expect(indexedInput.value)
    .eql("Hello")
    .expect(documentCookie.textContent)
    .contains("sps-userName=%22John%22")
    .expect(undefinedValue.textContent)
    .eql("undefined")
    .expect(undefinedType.textContent)
    .eql("undefined")
    .expect(undefinedValue2.textContent)
    .eql("undefined")
    .expect(undefinedType2.textContent)
    .eql("undefined")
    .expect(undefinedValue3.textContent)
    .eql("undefined")
    .expect(undefinedType3.textContent)
    .eql("undefined")
    .expect(arrayValue.textContent)
    .eql("1,2,3")
    .expect(arrayType.textContent)
    .eql("object")
    .expect(objectValue.textContent)
    .eql("[object Object]")
    .expect(objectType.textContent)
    .eql("object")
    .expect(nullValue.textContent)
    .eql("null")
    .expect(nullType.textContent)
    .eql("object")
    .expect(nullValue2.textContent)
    .eql("null")
    .expect(nullType2.textContent)
    .eql("object")
    .expect(nullValue3.textContent)
    .eql("null")
    .expect(nullType3.textContent)
    .eql("object")
    .expect(classValue.textContent)
    .eql("John")
    .expect(className.textContent)
    .eql("NameHolder")
    .expect(storageValue.textContent)
    .eql('{"foo":"bar","baz":{"hello":"world","foobar":[1,2,3]}}')
    .expect(storageRawValue.textContent)
    .eql(
      '{"foo":"bar","baz":{"hello":"world","foobar":["#$@__reference__2",1,2,3],"#$@__reference__":1},"#$@__reference__":0}'
    )
})
...
test("Cookie storage", async (t) => {
  await t
    ...
    .expect(documentCookie.textContent)
    .contains("sps-userName=%22Foobar%22")
    .click(reloadButton)
    .expect(cookieInput.value)
    .eql("Foobar")
    .expect(documentCookie.textContent)
